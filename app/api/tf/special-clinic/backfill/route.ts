 ...
 * TF 정보가 없는 메시지(구식 포맷)는 DB 환자명 매칭으로 TF 추론 → 실패 시 '미확인TF'.
 * 파일 1개 기준 요청 (프론트가 여러 파일을 순차 호출).
 */
export const maxDuration = 300
export const runtime = 'nodejs'

const LEGACY_TF = '미확인TF'
...
export async function POST(req: NextRequest) {
  ...
  if (missingTfNames.size > 0) {
    ...
    for (const p of patients) {
      if (nameCount.get(p.name) !== 1) continue // 동명이인은 애매 → 미확인TF
  ...
  return NextResponse.json({
    ...
    stats: {
      ...
      legacyTagged,   // TF 없어서 미확인TF로 저장된 건수
    },
  })
}
