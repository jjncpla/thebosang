...
1인 + 직원 수동 테스트의 한계를 보완하기 위한 specialist. 호출 시점에 작업하며, "상시" 모니터링은 별도 트리거(GitHub Actions / Claude Code schedule / Railway 크론)로 본 에이전트를 자동 호출하는 방식으로 구현됨.
...
## 상시 트리거 (별도 작업 — 본 에이전트 정의에 포함되지 않음)

본 에이전트는 호출 시점에만 작동한다. "상시" 작동시키려면 다음 트리거 메커니즘이 별도로 필요:

| 메커니즘 | 시점 | 호출 작업 |
|---------|------|----------|
| **GitHub Actions** | PR open / push to main | 변경된 파일 보안 리뷰 + lint/build 결과 분석 |
| **Claude Code schedule** | 매일 아침 / 매주 | 어제 머지 커밋 점검, Phase 진행 상태 vs 기획서 비교 |
| **Railway 크론** | 5분/1시간 간격 | `/api/_health` 호출, 응답 이상 시 알림 |

> 트리거 셋업은 별도 PR. 본 에이전트 정의 파일은 **호출됐을 때 무엇을 할지**만 명시.
